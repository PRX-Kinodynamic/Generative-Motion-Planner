...
def combine_plans_trajectories(plans, trajectories):
    ...
    for plan, trajectory in zip(plans, trajectories):
        combined.append(np.concatenate([trajectory[: -1, :], plan], axis=-1))
    return combined
...
#         if plan is None:
#             continue
#         trajectory = trajectories[index][:-1]
#         if trajectory is None:
#             continue
#         assert len(plan) == len(trajectory), f"Plan length {len(plan)} is not equal to trajectory length {len(trajectory)}"
        
#         combined.append(np.concatenate((trajectory, plan), axis=-1))
