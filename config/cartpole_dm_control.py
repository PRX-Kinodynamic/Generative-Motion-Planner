...
mins = [  -2.0699819 , -np.pi,   -8.62452465,  -18.5882527 ]
maxs = [  2.0699819, np.pi,   8.62452465,  18.5882527]
...
def attractor_classification_fn(final_states: np.ndarray, attractors: dict, attractor_dist_threshold: float, invalid_label: int = -1, verbose: bool = True):
    if verbose:
        print("[ config/cartpole ] Getting attractor labels for trajectories")
...
def read_trajectory(sequence_path):
    ...
    for i, line in enumerate(lines):
        ...
        if line == "":
            if i < len(lines) - 1:
                raise ValueError(f"[ config/cartpole ] Empty line found at {sequence_path} at line {i}")
            else:
                break
        ...
        if len(state) < 4:
            raise ValueError(f"[ config/cartpole ] Trajectory at {sequence_path} has less than 4 states at line {i}")
