"""
Configuration for 2D Quadrotor RL environment (PyBullet).

This config contains training/model setup. System-specific details
(state limits, manifolds, preprocessing) are provided by Quadrotor2DRLSystem.

Dataset: quadrotor2D_rl
State: [x, z, theta, x_dot, z_dot, theta_dot]
Goal: Stabilize to hover at (0, 1, 0, 0, 0, 0)
...
from genMoPlan.systems import Quadrotor2DRLSystem
...
def get_system(config=None, use_manifold: bool = False, dataset: str = None, **kwargs):
    """
    Create a Quadrotor2DRLSystem from this config.
...
base = {
    ...
    "base": {
        ...
        # ---------------------------- training ----------------------------#
        # Large dataset (~490k trajectories) - fewer epochs needed
        "num_epochs": 50,
    ...
    "diffusion": {
        ...
        "stride": 1,
        "model_kwargs": {
            # Scaled up from 32 for 6D state space and large dataset (~490k trajectories)
            "base_hidden_dim": 64,
...
larger_hidden_dim = {
    # Even larger model if 64 is insufficient
    "model_kwargs": {
        "base_hidden_dim": 128,
    },
}
...
dit_test = {
    # DiT sized for 6D state space and large dataset (~490k trajectories)
    "model": "models.temporal.TemporalDiffusionTransformer",
    "model_kwargs": {
        "hidden_dim": 192,  # Scaled up from 128 for 6D state
        "num_layers": 6,  # More layers for complex dynamics
        "num_heads": 6,
