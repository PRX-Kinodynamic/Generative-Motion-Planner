...
class Classifier:
    ...
    ground_truth_final_states: np.ndarray = None  # From test_set.txt
    n_runs: int = None
    _num_expected_runs: int = None
    ...
    final_state_error_results: dict = None  # Results from compute_final_state_errors
    ...
    def __init__(
        ...
        self.device = device
        if system is None:
            raise ValueError("Classifier requires a `system` instance; system=None is not supported.")
        self.system = system

        self._orig_n_runs = n_runs

        self.inference_params = load_inference_params(self.dataset, system=self.system)
        ...
        # Single source of truth for step parameters is always the system
    ...
    def _load_params(self):
        ...
        self._num_expected_runs = self.n_runs
        self.batch_size = (
            self.batch_size
    ...
    def timestamp(self, value):
        if self._trajectory_generator is None:
            raise ValueError(
                "Cannot set timestamp without a trajectory generator (model_path required)"
            )
    ...
    def enable_validation_mode(self):
        ...
        self.inference_params["final_state_directory"] = self._final_states_dir_name

        if self._trajectory_generator is not None:
            self._trajectory_generator.final_state_directory = self._final_states_dir_name

        if self.verbose:
            print(f"[ utils/classifier ] Validation mode enabled")
            print(f"  Results dir: {self._results_dir_name}/")
            print(f"  Final states dir: {self._final_states_dir_name}/")
    ...
    def _setup_results_path(self):
        ...
        if callable(self.inference_params["results_name"]):
            ...
            self.results_path = path.join(
                self.model_path, self._results_dir_name, self.inference_params["results_name"]
            )
        ...
        try:
            ...
            if self.verbose:
                print("[ utils/classifier ] Failed to save system_state.json")
    ...
    def _save_inference_params(self, save_path: str = None):
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Inference params saved in {json_fpath}")
    ...
    def _save_predicted_labels(self):
        ...
        with open(file_path, "w") as f:
            ...
            for start_point_idx in range(self.start_states.shape[0]):
                ...
                f.write("\n")
        if self.verbose:
            print(f"[ utils/classifier ] Saved predicted labels to {file_path}")
    ...
    def reset(self, for_analysis: bool = False):
        ...
        **Set/Modified attributes:**
        ...
        self.final_state_error_results = None
    ...
    def save_final_states(self):
        ...
        **Required attributes:**
        ...
        """
        if self.verbose:
            print(f"[ utils/classifier ] Saving final states")
        ...
        generator.save_final_states(
            ...
            metadata_n_runs=self._num_expected_runs,
        )
    ...
    def plot_trajectories(self):
        ...
        **Required attributes:**
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Plotting trajectories")
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Trajectories plotted in {image_path}")
    ...
    def plot_roas(self, plot_separatrix=True, s=0.1):
        ...
        **Required attributes:**
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Plotting ROAs")
        if self.predicted_labels is None:
            raise ValueError("No predicted labels available")
    ...
    def plot_classification_results(self, s=0.1):
        ...
        **Required attributes:**
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Plotting classification results")
        ...
        if self.verbose:
            print(
                f"[ utils/classifier ] Classification result plots saved in {self.results_path}"
            )
    ...
    def load_ground_truth(self):
        """
        Loads ground truth start states, final states, and expected labels from a file.
        ...
        **Required attributes:**
            ...
            - `system`: System instance (for state_dim)

        **Set/Modified attributes:**
            - `start_states`: Loaded start states.
            - `ground_truth_final_states`: Ground truth final states (from test_set.txt only).
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Loaded test_set.txt with ground truth final states")
        ...
        if self.verbose:
            print(
                f"[ utils/classifier ] Loaded {self.start_states.shape[0]} ground truth data points"
            )

    def load_validation_ground_truth(self):
        """
        Loads validation trajectories as ground truth for evaluation.

        This is an alternative to load_ground_truth() that uses the validation
        dataset trajectories instead of test_set.txt. The initial state is the
        first state of each trajectory, the final state is the last state, and
        labels are loaded from the corresponding shuffled_labels file.

        This method automatically enables validation mode, which uses separate
        output directories (results_val/, final_states_val/).

        The val_dataset_size and shuffled_indices_fname are read from the saved
        model args (args.json). If shuffled_indices_fname is not present, defaults
        to "shuffled_indices.txt".

        **Required attributes:**
            - `dataset`: Name of the dataset
            - `system`: System instance (for read_trajectory and state_dim)
            - `model_args`: Saved model args containing val_dataset_size

        **Set/Modified attributes:**
            - `start_states`: Initial states from each validation trajectory.
            - `ground_truth_final_states`: Final states from each validation trajectory.
            - `expected_labels`: Labels loaded from shuffled_labels file.
            - `batch_size`: May be modified if `num_batches` is set.

        Raises:
            FileNotFoundError: If shuffled indices/labels files are not found.
            ValueError: If system is not provided or doesn't have read_trajectory.
        """
        if self.system is None:
            raise ValueError("System is required to load validation ground truth.")

        if not hasattr(self.system, "read_trajectory") or self.system.read_trajectory is None:
            raise ValueError(
                "System must have a read_trajectory method to load validation trajectories."
            )

        if self.model_args is None:
            raise ValueError(
                "model_args is required to load validation ground truth. "
                "Ensure model_path is provided when creating the Classifier."
            )

        # Get val_dataset_size from model_args
        ...
        # Get shuffled_indices_fname from model_args, default to "shuffled_indices.txt"
        shuffled_indices_fname = getattr(
            self.model_args, "shuffled_indices_fname", "shuffled_indices.txt"
        )

        # Enable validation mode (separate output directories)
        self.enable_validation_mode()

        if self.verbose:
            print("[ utils/classifier ] Loading validation ground truth")
            print(f"  val_dataset_size: {val_dataset_size}")
            print(f"  shuffled_indices_fname: {shuffled_indices_fname}")
        ...
        # Load labels from corresponding shuffled_labels file
        ...
        # Load all labels and get the last val_dataset_size
        with open(labels_fpath, "r") as f:
            all_labels = [int(line.strip()) for line in f.readlines()]
        labels = np.array(all_labels[-val_dataset_size:], dtype=np.int32)

        # Extract start states (first state) and final states (last state)
        start_states = np.array([traj[0] for traj in trajectories], dtype=np.float32)
        final_states = np.array([traj[-1] for traj in trajectories], dtype=np.float32)

        self.start_states = start_states
        self.ground_truth_final_states = final_states
        ...
        if self.verbose:
            print(
                f"[ utils/classifier ] Loaded {self.start_states.shape[0]} validation data points"
            )
            print(f"  Start states shape: {self.start_states.shape}")
            print(f"  Final states shape: {self.ground_truth_final_states.shape}")
    ...
    def generate_trajectories(
        ...
        num_existing_runs = 0 if self.final_states is None else self.final_states.shape[1]
        num_runs_needed = self._num_expected_runs - num_existing_runs
        num_total_runs = self.start_states.shape[0] * num_runs_needed

        if num_runs_needed <= 0:
            if self.verbose:
                print(
                    f"[ utils/classifier ] Skipping generation; already have {num_existing_runs} runs."
                )
            return self.final_states

        if num_existing_runs > 0:
            if self.verbose:
                print(
                    f"[ utils/classifier ] Continuing from run {num_existing_runs + 1}/{self._num_expected_runs}"
                )
        if num_existing_runs > 0 and return_trajectories:
            raise ValueError("Cannot return trajectories when continuing from previous data.")

        if num_total_runs > 1e4 and return_trajectories:
            warnings.warn(
                f"Generating and storing {num_total_runs} trajectories may consume a lot of memory."
            )
            
        final_state_runs, trajectory_runs = self.trajectory_generator.generate_multiple_runs(
            start_states=self.start_states,
            n_runs=num_runs_needed,
            batch_size=self.batch_size,
            return_trajectories=return_trajectories,
            save=save,
            run_offset=num_existing_runs,
            metadata_n_runs=self._num_expected_runs,
        ...
        generated_final_states = final_state_runs.transpose(1, 0, 2) # (n_runs, num_start_states, state_dim) -> (num_start_states, n_runs, state_dim)

        if num_existing_runs > 0 and self.final_states is not None:
        ...
        **Set/Modified attributes:**
        ...
        final_states_run_first, loaded_runs, resolved_timestamp = (
            self.trajectory_generator.load_saved_final_states(
                expected_runs=self._num_expected_runs,
                timestamp=timestamp,
                parallel=parallel,
            )
        )
        ...
        if self.n_runs != self._num_expected_runs:
            warnings.warn(
                f"Loaded {self.n_runs} runs instead of {self._num_expected_runs}"
            )
    ...
    def compute_final_state_errors(self, save: bool = True) -> dict:
        """
        Compute final state error statistics comparing predictions to ground truth.

        Uses manifold-aware distance computation (system.manifold.dist) for proper
        handling of angular components.

        **Required attributes:**
            - `final_states`: Predicted final states shape (N, n_runs, state_dim)
            - `ground_truth_final_states`: Ground truth final states shape (N, state_dim)
            - `system`: System with manifold for distance computation

        **Set/Modified attributes:**
            - `final_state_error_results`: Dictionary with all computed statistics

        Returns:
            dict: Error statistics with structure:
                - global: {mean, variance, p50, p90, p99} over all N*n_runs errors
                - per_initial_state: {mean, variance, p50, p90, p99} each with
                  {mean, std, p50, p90, p99} aggregate stats
                - per_dimension_mae: {dim_name: mae_value, ...}
                - metadata: {num_start_states, n_runs, state_dim}
        """
        if self.final_states is None:
            raise ValueError("No final states available. Generate or load first.")
        if self.ground_truth_final_states is None:
            raise ValueError("No ground truth final states. Load test_set.txt first.")
        if self.system is None:
            raise ValueError("System required for manifold-aware distance computation.")
        if getattr(self.system, "manifold", None) is None:
            raise ValueError(
                "system.manifold (true manifold) is required for final-state errors; "
                "no Euclidean fallback is allowed."
            )

        n_points, n_runs, state_dim = self.final_states.shape

        # Convert to torch for manifold.dist()
        pred_t = torch.from_numpy(self.final_states).float()  # (N, n_runs, D)
        gt_t = torch.from_numpy(self.ground_truth_final_states).float()  # (N, D)

        # Broadcast ground truth: (N, D) -> (N, 1, D) -> broadcast with (N, n_runs, D)
        gt_expanded = gt_t.unsqueeze(1)  # (N, 1, D)

        # Compute per-dimension manifold-aware distance
        # manifold.dist returns per-dimension distances for Product manifolds
        per_dim_error = self.system.manifold.dist(pred_t, gt_expanded)  # (N, n_runs, D)

        # Compute scalar error per prediction: sqrt(sum(per_dim^2))
        scalar_error = torch.sqrt(torch.sum(per_dim_error**2, dim=-1))  # (N, n_runs)
        ...
        def aggregate_stats(arr: np.ndarray) -> dict:
            return {
        ...
        per_initial_state_stats = {
            "mean": aggregate_stats(per_state_mean),
            "variance": aggregate_stats(per_state_var),
            "p50": aggregate_stats(per_state_p50),
            "p90": aggregate_stats(per_state_p90),
            "p99": aggregate_stats(per_state_p99),
        }
        ...
        results = {
            "global": global_stats,
            "per_initial_state": per_initial_state_stats,
            "per_dimension_mae": per_dimension_mae,
            "num_start_states": n_points,
            "n_runs": n_runs,
            "state_dim": state_dim,
        }

        self.final_state_error_results = results

        # Save to file
        if save:
            self._setup_results_path()
            results_fpath = path.join(self.results_path, "final_state_errors.json")
            with open(results_fpath, "w") as f:
                json.dump(results, f, indent=4)
            if self.verbose:
                print(f"[ utils/classifier ] Final state errors saved to {results_fpath}")

        # Print summary if verbose
        if self.verbose:
            print(f"\n[ utils/classifier ] Final State Error Statistics:")
            print(f"  Global (N={n_points}, n_runs={n_runs}):")
            print(f"    Mean:     {global_stats['mean']:.6f}")
            print(f"    Variance: {global_stats['variance']:.6f}")
            print(f"    P50:      {global_stats['p50']:.6f}")
            print(f"    P90:      {global_stats['p90']:.6f}")
            print(f"    P99:      {global_stats['p99']:.6f}")
            print(f"  Per-dimension MAE:")
            for name, mae in per_dimension_mae.items():
                print(f"    {name}: {mae:.6f}")
    ...
    def predict_outcomes(self, save=True):
        ...
        if save:
            ...
            if self.verbose:
                print(f"[ utils/classifier ] Saved predicted outcomes to {file_path}")
    ...
    def load_predicted_labels(self, results_path: str):
        ...
        **Set/Modified attributes:**
        ...
        """
        if self.verbose:
            print(f"[ utils/classifier ] Loading predicted labels")
        ...
        if self.verbose:
            print(f"[ utils/classifier ] Loaded {len(self.predicted_labels)} predicted labels")
    ...
    def load_predicted_outcomes(self, results_path: str):
        ...
        """
        if self.verbose:
            print(f"[ utils/classifier ] Loading predicted outcomes")
        ...
        if self.verbose:
            print(
                f"[ utils/classifier ] Loaded {len(self.predicted_outcomes)} predicted outcomes"
            )
    ...
    def compute_classification_results(self, save=True):
        ...
        Calculates metrics like accuracy, precision, recall, specificity, TP, FP, TN, FN.
        ...
        **Set/Modified attributes:**
        ...
        # Count separatrix predictions (predictions that couldn't be classified as success/failure)
        # among samples with valid ground truth
        n_separatrix = np.sum(
            predicted_labels_for_valid_gt == self.labels_array[-1]
        )

        # Break down separatrix predictions into:
        # 1. uncertain: neither success nor failure prob above threshold
        # 2. invalid_outcome: an invalid outcome (e.g., out-of-bounds) had high probability
        # These are tracked in predict_outcomes() as outcome_uncertain_indices and outcome_invalid_indices
        outcome_uncertain_indices = getattr(self, "outcome_uncertain_indices", None)
        outcome_invalid_indices = getattr(self, "outcome_invalid_indices", None)

        # Count uncertain predictions (within valid ground truth samples)
        if outcome_uncertain_indices is not None and len(outcome_uncertain_indices) > 0:
            # Filter to only those with valid ground truth
            n_uncertain = np.sum(valid_gt_mask[outcome_uncertain_indices])
        else:
            n_uncertain = 0

        # Count invalid outcome predictions (within valid ground truth samples)
        if outcome_invalid_indices is not None and len(outcome_invalid_indices) > 0:
            # Filter to only those with valid ground truth
            n_invalid_outcome = np.sum(valid_gt_mask[outcome_invalid_indices])
        else:
            n_invalid_outcome = 0
        ...
        # Rates (conditional probabilities)
        tp_rate = tp / (tp + fn) if (tp + fn) > 0 else np.nan  # Recall / Sensitivity
        tn_rate = tn / (tn + fp) if (tn + fp) > 0 else np.nan  # Specificity
        fp_rate = fp / (fp + tn) if (fp + tn) > 0 else np.nan  # False Positive Rate
        fn_rate = fn / (fn + tp) if (fn + tp) > 0 else np.nan  # False Negative Rate

        total_valid = int(n_separatrix) + int(tp) + int(tn) + int(fp) + int(fn)
        separatrix_rate = int(n_separatrix) / total_valid if total_valid > 0 else np.nan
        uncertain_rate = int(n_uncertain) / total_valid if total_valid > 0 else np.nan
        invalid_outcome_rate = int(n_invalid_outcome) / total_valid if total_valid > 0 else np.nan

        # Metrics (excluding invalid predictions from accuracy calculation)
        valid_decisions = tp + tn + fp + fn
        accuracy = (tp + tn) / valid_decisions if valid_decisions > 0 else np.nan
        precision = tp / (tp + fp) if (tp + fp) > 0 else np.nan
        recall = tp / (tp + fn) if (tp + fn) > 0 else np.nan  # Same as tp_rate
        specificity = tn / (tn + fp) if (tn + fp) > 0 else np.nan  # Same as tn_rate
        f1_score = (
        ...
        # Labels: [Negative, Positive, Uncertain, Invalid Outcome]
        # Compute uncertain and invalid outcome counts per actual class
        actual_neg_mask = expected_labels_valid == self.labels_array[0]
        actual_pos_mask = expected_labels_valid == self.labels_array[1]

        # Get indices of samples with valid ground truth for each actual class
        valid_gt_indices = np.where(valid_gt_mask)[0]

        # Uncertain per actual class
        if outcome_uncertain_indices is not None and len(outcome_uncertain_indices) > 0:
            uncertain_in_valid = np.isin(valid_gt_indices, outcome_uncertain_indices)
            uncertain_for_actual_neg = np.sum(uncertain_in_valid & actual_neg_mask)
            uncertain_for_actual_pos = np.sum(uncertain_in_valid & actual_pos_mask)
        else:
            uncertain_for_actual_neg = 0
            uncertain_for_actual_pos = 0

        # Invalid outcome per actual class
        if outcome_invalid_indices is not None and len(outcome_invalid_indices) > 0:
            invalid_in_valid = np.isin(valid_gt_indices, outcome_invalid_indices)
            invalid_for_actual_neg = np.sum(invalid_in_valid & actual_neg_mask)
            invalid_for_actual_pos = np.sum(invalid_in_valid & actual_pos_mask)
        else:
            invalid_for_actual_neg = 0
            invalid_for_actual_pos = 0

        confusion_matrix = [
            [int(tn), int(fp), int(uncertain_for_actual_neg), int(invalid_for_actual_neg)],  # Actual negative
            [int(fn), int(tp), int(uncertain_for_actual_pos), int(invalid_for_actual_pos)],  # Actual positive
        ]

        results = {
            "confusion_matrix": confusion_matrix,
            "confusion_matrix_labels": ["Negative", "Positive", "Uncertain", "InvalidOutcome"],
            "true_positives": int(tp),
            "true_negatives": int(tn),
            "false_positives": int(fp),
            "false_negatives": int(fn),
            "separatrix": int(n_separatrix),
            "separatrix_uncertain": int(n_uncertain),
            "separatrix_invalid_outcome": int(n_invalid_outcome),
            ...
            "invalid_outcome_rate": float(invalid_outcome_rate),
        ...
        if save:
            ...
            if self.verbose:
                print(f"[ utils/classifier ] Classification results saved to {results_fpath}")

        if self.verbose:
            self._print_classification_results(results)

        return results

    def _print_classification_results(self, results: dict):
        """Pretty-print classification results."""
        print(f"\n{'='*60}")
        print(f"Classification Results | {self.n_runs} runs")
        print(f"{'='*60}")

        # Confusion Matrix
        print(f"\nConfusion Matrix (rows=actual, cols=predicted):")
        col_labels = results["confusion_matrix_labels"]  # [Negative, Positive, Uncertain, InvalidOutcome]
        row_labels = ["Negative", "Positive"]  # actual labels (separatrix not in ground truth)
        cm = results["confusion_matrix"]

        # Header
        actual_pred = "Actual \\ Pred"
        header = f"{actual_pred:>14} |"
        ...
        # Totals row
        totals = [sum(cm[i][j] for i in range(len(row_labels))) for j in range(len(col_labels))]
        ...
        # Counts
        print(f"\nCounts:")
        print(f"  True Positives (TP):      {results['true_positives']:>8}")
        print(f"  True Negatives (TN):      {results['true_negatives']:>8}")
        print(f"  False Positives (FP):     {results['false_positives']:>8}")
        print(f"  False Negatives (FN):     {results['false_negatives']:>8}")
        print(f"  Separatrix:               {results['separatrix']:>8}")
        print(f"    - Uncertain:            {results['separatrix_uncertain']:>8}  (no outcome above threshold)")
        print(f"    - Invalid Outcome:      {results['separatrix_invalid_outcome']:>8}  (e.g., out-of-bounds)")

        # Rates
        print(f"\nRates:")
        print(f"  TP Rate (Sensitivity):    {results['tp_rate']:>8.4f}")
        print(f"  TN Rate (Specificity):    {results['tn_rate']:>8.4f}")
        print(f"  FP Rate:                  {results['fp_rate']:>8.4f}")
        print(f"  FN Rate:                  {results['fn_rate']:>8.4f}")
        print(f"  Separatrix Rate:          {results['separatrix_rate']:>8.4f}")
        print(f"    - Uncertain Rate:       {results['uncertain_rate']:>8.4f}")
        print(f"    - Invalid Outcome Rate: {results['invalid_outcome_rate']:>8.4f}")

        # Metrics
        print(f"\nMetrics:")
        print(f"  Accuracy:                 {results['accuracy']:>8.4f}")
        print(f"  Precision:                {results['precision']:>8.4f}")
