    ...
    try:
        ...
        for key, loss_list in losses.items():
            if loss_list:  # Check if the list is not empty
                ...
                if positive_losses:
                    ax2.plot(
                        positive_steps, positive_losses, label=key
                    )  # Plot each loss type
        ax2.set_yscale("log")
...
class Trainer(object):
    ...
    def __init__(
        ...
        self.num_steps_per_epoch = max(
            ceil(len(train_dataset) / (batch_size * gradient_accumulate_every)),
            min_num_steps_per_epoch,
        )

        print(
            f"[ utils/training ] Number of steps per epoch: {self.num_steps_per_epoch}"
        )

        if val_dataset is not None:
            self.val_num_batches = ceil(len(val_dataset) / (val_batch_size))
            print(
                f"[ utils/training ] Number of validation batches: {self.val_num_batches}"
            )
        else:
    ...
    def _plot_in_background(self, losses: dict, filepath: str, title: str):
        ...
        process = multiprocessing.Process(
            target=plot_losses, args=(losses_copy, filepath, title)
        )
        process.start()
    ...
    def add_train_losses(self, losses):
        for key, loss in losses.items():
            ...
            if isinstance(loss, (float, int)):
                ...
                print(
                    f"Warning: Skipping non-scalar loss value for key '{key}' in train_losses: {loss}"
                )

        if (
            self.step % self.log_freq == 0 and self.step > 0
        ):  # Avoid plotting at step 0 if empty
    ...
    def add_eval_losses(self, losses):
        for key, loss in losses.items():
            ...
            if isinstance(loss, (float, int)):
                ...
                print(
                    f"Warning: Skipping non-scalar loss value for key '{key}' in eval_losses: {loss}"
                )
    ...
    def add_val_losses(self, losses):
        for key, loss in losses.items():
            ...
            if isinstance(loss, (float, int)):
                ...
                print(
                    f"Warning: Skipping non-scalar loss value for key '{key}' in val_losses: {loss}"
                )
    ...
    def load(self, model_state_name: str = "best.pt"):
        ...
        self.num_steps_per_epoch = data.get(
            "num_steps_per_epoch", self.num_steps_per_epoch
        )
    ...
    def train_one_epoch(self, store_losses: bool = True):
        ...
        while (self.step - steps_start) < self.num_steps_per_epoch:
            ...
            if self.step % self.log_freq == 0:
                ...
                if info_items:
                    infos_str = " | " + " | ".join(
                        [f"{key}: {val:8.4f}" for key, val in info_items]
                    )
                else:
                    infos_str = ""
                print(
                    f'    Step {self.step}: train_loss={train_losses["train_loss"]:8.6f}{infos_str}  (t={timer():8.3f}s)',
                    flush=True,
                )
    ...
    def train(self, reraise_keyboard_interrupt: bool = False):
        ...
        try:
            for epoch in range(1, self.num_epochs + 1):
                ...
                planned_total_steps = (
                    self.lr_scheduler.total_steps
                    if self.lr_scheduler is not None
                    else self.num_epochs * self.num_steps_per_epoch
                )
                if self.step >= planned_total_steps:
                    print(
                        f"Reached planned total steps ({planned_total_steps}); stopping."
                    )
                    break
                ...
                if (
                    hasattr(self.model_args, "used_variations")
                    and self.model_args.used_variations
                ):
                ...
                if "unnorm_val_mae" in val_losses:
                    print(
                        f"    Unnormalized trajectory MAE: {val_losses['unnorm_val_mae']:.6f}"
                    )
                    ...
                    if state_names is not None:
                        per_dim_str = ", ".join(
                            [
                                f"{name}: {val_losses.get(f'unnorm_val_mae_{name}', 0):.4f}"
                                for name in state_names
                                if f"unnorm_val_mae_{name}" in val_losses
                            ]
                        )
                        if per_dim_str:
                ...
                if "unnorm_final_horizon_step_mae" in val_losses:
                    print(
                        f"    Final horizon step MAE:      {val_losses['unnorm_final_horizon_step_mae']:.6f}"
                    )
                    if state_names is not None:
                        per_dim_str = ", ".join(
                            [
                                f"{name}: {val_losses.get(f'unnorm_final_horizon_step_mae_{name}', 0):.4f}"
                                for name in state_names
                                if f"unnorm_final_horizon_step_mae_{name}" in val_losses
                            ]
                        )
                        if per_dim_str:
                            print(f"      Per-dim: {per_dim_str}")
                ...
                if (
                    self.perform_final_state_evaluation
                    and (epoch % self.eval_freq) == 0
                ):
                    final_rollout_mae, final_rollout_infos = (
                        self.evaluate_final_states()
                    )
                    ...
                    if state_names is not None:
                        per_dim_str = ", ".join(
                            [
                                f"{name}: {final_rollout_infos.get(f'final_rollout_mae_{name}', 0):.4f}"
                                for name in state_names
                                if f"final_rollout_mae_{name}" in final_rollout_infos
                            ]
                        )
                        if per_dim_str:
                            print(f"      Per-dim: {per_dim_str}")
                ...
                if val < best_val:
                    pos_impr = (
                        max(improvement, 0.0) if best_val != float("inf") else 0.0
                    )
                ...
                # Early stopping info
                print(
                    f"  Early stopping: window_gain={window_gain:.4g} / target={self.min_delta}"
                )
                ...
                if self.early_stopping and epoch > self.warmup_epochs:
                    if len(recent_improvements) == self.patience:
                        ...
                        if window_gain < self.min_delta:
                            ...
                            print(
                                f"  Window gain ({window_gain:.4g}) < target ({self.min_delta})"
                            )
    ...
    def evaluate_final_states(self, store_losses: bool = True):
        ...
        target_final_states_normalized_np = (
            eval_data.final_states.detach().cpu().numpy()
        )
        if normalizer is not None:
            target_final_states = normalizer.unnormalize(
                target_final_states_normalized_np
            )
        else:
        ...
        if (
            abs_error.ndim == 2
            and abs_error.shape[-1] == predicted_final_states.shape[-1]
        ):
        ...
        with torch.no_grad(), self.swap_to_ema():
            for i, batch in enumerate(self.dataloader_val):
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    print(
                        f"Warning: Invalid validation loss detected (NaN/Inf) at batch {i}. Skipping."
                    )
                    continue
